...
const appendProduct = (product, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ message: "Los campos deben estar completos." });
  }
...
router.post("/", (req, res) => {
  ...
  if (!products) {
    ...
    res
      .status(201)
      .json({ message: "Producto creado correctamente.", product: req.body });
  } else {
    try {
      ...
      res
        .status(201)
        .json({ message: "Productos creados correctamente.", products });
    } catch (error) {
      res.status(400).json({ message: error.message });
    }
  }
...
router.put("/:id", getProduct, async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ message: "Debe enviar todos los campos para actualizar." });
  }
